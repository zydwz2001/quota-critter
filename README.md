...
当前版本 **0.1.4**，尚未发布到 GitHub Release；最新公开版仍是 [v0.1.1](https://github.com/zydwz2001/quota-critter/releases/tag/v0.1.1)。首次运行前请先在 Codex CLI 或官方 VS Code Codex 扩展中登录。若只安装了 VS Code 扩展，Quota Critter 会自动发现扩展内置的 Codex，无需再单独安装 CLI。

## 安装下载

- **最新公开版（v0.1.1）**：[GitHub Releases](https://github.com/zydwz2001/quota-critter/releases/tag/v0.1.1)（macOS DMG / Windows x64 NSIS）。
- **本地构建最新版**：见下方「本地运行」一节用 `npm run tauri build` 生成。
...
先确认本机已经登录 Codex CLI，或已经安装并登录官方 VS Code Codex 扩展，然后在项目根目录执行：
